...
#Veriscript_TB BETA 1.1 08/31/2023
...
def eliminarComentarios(nombre):
    ...
    while(True):
        ...
        lineas.append(linea)
    archivo.close()
    for it in range(len(lineas)):
        if "//" in lineas[it]:
            split = lineas[it].split("//")
            if split == None: 
                continue
            code = split[0]
            if split[0] == '': 
    ...
    for linea in lineas:
        if linea != '':
            lineasConAlgo.append(linea)
    return lineasConAlgo

def extraer(nombre):
    lineas=eliminarComentarios(nombre)
    nam_module=""
    stauxlist=[]
    stauxlist2=[]
    entradas=[]
    ...
    return nam_module,entradas_f,salidas_f
    
def escribir(folder,nam_module,entradas,salidas):
    texto=""
    clock=""
    listaux=[]
    entradas_bit=[]
    ...
    salidas_f=[]
    aux=0
    nombre=folder+nam_module+"_TB_SCRPT.v"
    archivo=open(nombre,mode="w",encoding="utf-8")
    archivo.write("//Archivo de TestBench para verilog basico generado por Veriscript_TB BETA 1.1\n")
...
def getFolder(nombre):
    listOfFolder = nombre.split("\\")
    folder=""
...
if __name__ == "__main__":
    ...
    nombre = leer()
    folder = getFolder(nombre)
    nam_module,entradas_f,salidas_f = extraer(nombre)
    ruta = escribir(folder,nam_module,entradas_f,salidas_f)
    print("\nARCHIVO CREADO CON EXITO! \nPuede encontrarlo en: " + ruta )
